...
mo_core_proj, mo_core_comp = ao_split(mol, mo_core, ["O1 2s", "Cu1 3[sp]", "O[23] 2[sp]"], s=s)
...
mo_coeff_new_s0 = np.hstack((mo_core_comp, mo_core_proj, mo_cas, mo_virt_proj, mo_virt_comp, mo_proj))

molden.from_mo(mol, project + "_ptorb_s0.molden", np.hstack((mo_core_proj, mo_virt_proj)), occ=np.zeros(mo_core_proj.shape[1] + mo_virt_proj.shape[1]))
molden.from_mo(mol, project + "_mowith_ptorb_s0.molden", mo_coeff_new_s0, occ=mf.mo_occ)
...
mo_virt_proj, mo_virt_comp = ao_split(mol, mo_virt, ["O1 3[sd]", "Cu1 4[spf]", "Cu1 [56]", "La1 5d", "La1 6", "O[23] 3[spd]"], s=s)
logger.info(mf, "Virt-correlated orbitals = %i" % mo_virt_proj.shape[1])
mo_coeff_new_s1 = np.hstack((mo_core_comp, mo_core_proj, mo_cas, mo_virt_proj, mo_virt_comp, mo_proj))

molden.from_mo(mol, project + "_ptorb_s1.molden", np.hstack((mo_core_proj, mo_virt_proj)), occ=np.zeros(mo_core_proj.shape[1] + mo_virt_proj.shape[1]))
molden.from_mo(mol, project + "_mowith_ptorb_s1.molden", mo_coeff_new_s1, occ=mf.mo_occ)
